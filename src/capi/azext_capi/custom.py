...
def init_environment(cmd, prompt=True, management_cluster_name=None,
    ...
    try:
        ...
        if prompt and not prompt_y_n("Do you want to use default kubernetes management cluster?"):
            use_new_cluster = True
        else:
            return True
    except ResourceNotFoundError as err:
        error_msg = err.error_msg
        if missing_components_match_condition(error_msg):
    ...
    with Spinner(cmd, "Waiting for Cluster API to be ready", "✓ Cluster API is ready"):
        ...
        for _ in range(0, 10):
            try:
                ...
                if missing_components_match_condition(last_err_msg):
                    raise
                time.sleep(delay)
        else:
...
def missing_components_match_condition(output):
...
def find_management_cluster():
    ...
    for component in components:
        check_required_management_namespace(component["namespace"])
        check_pods_status_by_namespace(component["namespace"], component["err_msg"], component["pod"])


def check_required_management_namespace(namespace):
...
def check_pods_status_by_namespace(namespace, error_message, pod_name):
    ...
    cmd = base_cmd + ["--namespace", namespace]
    try:
        output = run_shell_command(cmd)
